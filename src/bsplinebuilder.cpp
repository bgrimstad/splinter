...
namespace SPLINTER
{
    ...
    BSpline BSpline::Builder::build() const
    {
        ...
        // Build knot vectors
        auto knotVectors = computeKnotVectors(_data, _degrees);
    ...
    DenseMatrix BSpline::Builder::computeCoefficients(const DataTable &data, const BSpline& bspline) const
    {
        switch (_smoothing)
        {
            case Smoothing::NONE:
            ...
            case Smoothing::REGULARIZATION:
                throw Exception("BSpline::Builder::computeCoefficients: Regularization is not implemented.");
                return computeBSplineCoefficients(data, bspline);
    ...
    DenseMatrix BSpline::Builder::computeBSplineCoefficients(const DataTable &data, const BSpline& bspline) const
    {
        /*
    ...
    SparseMatrix BSpline::Builder::computeBasisFunctionMatrix(const DataTable &samples, const BSpline &bspline) const
    {
    ...
    DenseMatrix BSpline::Builder::controlPointEquationRHS(const DataTable &samples) const
    {
    ...
    */
    DenseMatrix BSpline::Builder::computePSplineCoefficients(const DataTable &samples, const BSpline &bspline) const
    {
    ...
    */
    ...
    std::vector<std::vector<double> > BSpline::Builder::computeKnotVectors(const DataTable &data, std::vector<unsigned int> degrees) const
    {
        if (data.getNumVariables() != degrees.size())
        ...
        for (unsigned int i = 0; i < data.getNumVariables(); ++i)
        {
    ...
    std::vector<double> BSpline::Builder::computeKnotVector(const std::vector<double> &values, unsigned int degree) const
    {
        switch (_knotSpacing)
        {
    ...
    */
    std::vector<double> BSpline::Builder::knotVectorMovingAverage(const std::vector<double> &values, unsigned int degree) const
    {
    ...
    std::vector<double> BSpline::Builder::knotVectorEquidistant(const std::vector<double> &values, unsigned int degree) const
    {
        // Sort and remove duplicates
        std::vector<double> unique = extractUniqueSorted(values);
