...
// Default constructor
BSplineBuilder::BSplineBuilder(const DataTable &data)
        : _data(data),
...
DenseMatrix BSplineBuilder::controlPointEquationRHS(const DataTable &samples) const
{
...
std::vector<std::vector<double> > BSplineBuilder::computeKnotVectors(const DataTable &data, std::vector<unsigned int> degrees) const
{
    if (data.getNumVariables() != degrees.size())
        throw Exception("BSplineBuilder::computeKnotVectors: Inconsistent sizes on input vectors.");
...
std::vector<double> BSplineBuilder::computeKnotVector(const std::vector<double> &values, unsigned int degree) const
{
    switch (_knotSpacing)
    {
        case BSplineKnotSpacing::SAMPLE:
            return knotVectorMovingAverage(values, degree);
        case BSplineKnotSpacing::EQUIDISTANT:
 ...
 */
std::vector<double> BSplineBuilder::knotVectorMovingAverage(const std::vector<double> &values, unsigned int degree) const
{
...
std::vector<double> BSplineBuilder::knotVectorEquidistant(const std::vector<double> &values, unsigned int degree) const
{
    // Sort and remove duplicates
    std::vector<double> unique = extractUniqueSorted(values);
...
std::vector<double> BSplineBuilder::knotVectorBuckets(const std::vector<double> &values, unsigned int degree, unsigned int maxSegments) const
{
    // Sort and remove duplicates
    std::vector<double> unique = extractUniqueSorted(values);
...
std::vector<double> BSplineBuilder::extractUniqueSorted(const std::vector<double> &values) const
{
    // Sort and remove duplicates
